...
void Lesson07::onEnter() {
	TextureData data = m_assetManager->loadTextureData("assets/nehe/crate.bmp");
...
void Lesson07::onUpdate(float deltaTime) {
    ...
    if (m_zRot > 360.0f) m_zRot -= 360.0f;
    if (m_xRot < -360.0f) m_xRot = 360.0f;
    if (m_yRot < -360.0f) m_yRot = 360.0f;
    if (m_zRot < -360.0f) m_zRot = 360.0f;
}

void Lesson07::onRenderUI() {
    ImGui::Text("Speed");
    ...
    if (ImGui::Combo("##filter", &m_currentFilter, filterLabels, IM_ARRAYSIZE(filterLabels))) {
    }
}

void Lesson07::onRender() {
