...
void Lesson09::onRender() {
  ...
  for (GLuint loop = 0; loop < NUM_STARS;
       loop++) {      // Loop Through All The Stars
    ...
    glRotatef(m_stars[loop].angle, 0.0f, 1.0f,
    ...
    if (twinkle) { // Twinkling Stars Enabled
      ...
      glColor4ub(m_stars[(NUM_STARS - loop) - 1].r,
                 m_stars[(NUM_STARS - loop) - 1].g,
                 m_stars[(NUM_STARS - loop) - 1].b, 255);
    ...
    m_stars[loop].angle +=
        float(loop) / NUM_STARS;     // Changes The Angle Of A Star
