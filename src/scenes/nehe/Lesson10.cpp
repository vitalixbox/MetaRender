...
void Lesson10::onUpdate(float deltaTime) {
  if (m_engine->getInputManager().isKeyDown(Key::Up) ||
      m_engine->getInputManager().isKeyDown(Key::W)) {
  ...
  if (m_engine->getInputManager().isKeyDown(Key::Down) ||
      m_engine->getInputManager().isKeyDown(Key::S)) {
    xpos += (float)sin(heading * piover180) * 0.05f;
    zpos += (float)cos(heading * piover180) * 0.05f;
  ...
  if (m_engine->getInputManager().isKeyDown(Key::Left) ||
      m_engine->getInputManager().isKeyDown(Key::A)) {
    heading += 1.0f;
    yrot = heading;
  }

  if (m_engine->getInputManager().isKeyDown(Key::Right) ||
      m_engine->getInputManager().isKeyDown(Key::D)) {
    heading -= 1.0f;
    yrot = heading;
  }
...
void Lesson10::onRender() {
  ...
  for (int loop_m = 0; loop_m < numtriangles;
       loop_m++) {                // Loop Through All The Triangles
    ...
    glVertex3f(x_m, y_m, z_m); // Set The TexCoord And Vertice
